...
class MemoizedEvaluator:
    ...
    def accuracy(self, data: numpy.ndarray, labels: numpy.ndarray) -> float:
        ...
        Args:
            ...
            labels: The labels.
        """
        return (self.tree.predict(data) == labels).sum() / labels.size
    ...
    def confusion_matrix(self, data: numpy.array, labels: numpy.ndarray) -> Tuple[float, float, float, float]:
        ...
        false_positive_rate = (stacked_labels.sum(axis=1) == 1).sum() / (labels == 0).sum()
        false_negative_rate = (stacked_labels.sum(axis=1) == 1).sum() / (labels == 1).sum()
